...
def prepare_federated_data(
    ...
    if os.path.exists(p1_path) and os.path.exists(p2_path):
        ...
        # logger.info("Performing IID split (stratified set to y)")
        # X_p1, X_p2, y_p1, y_p2 = train_test_split(
        #     X, y, test_size=initial_split_ratio, 
        #     stratify=y, 
        #     random_state=seed
        # )

        logger.info("Performing non-IID split (via skew ratio-based allocation)")
        unique_labels = np.unique(y)
        X_p1_list, y_p1_list = [], []
        X_p2_list, y_p2_list = [], []
        
        # --- Define skew ---
        split_label_index = len(unique_labels) // 2
        skew_factor = 0.9999 # e.g., 99.99% goes to the 'dominant' player for that class
        
        for i, label in enumerate(unique_labels):
            label_indices = np.where(y == label)[0]
            X_label = X[label_indices]
            y_label = y[label_indices] # All are the same label
        
            if not label_indices.size > 0: # Skip if class somehow has 0 samples
                continue
        
            # Determine the split ratio *for this specific label*
            if i < split_label_index:
                # Assign mostly to P1 (means small test_size for P2)
                current_p2_ratio = 1.0 - skew_factor
            else:
                # Assign mostly to P2 (means large test_size for P2)
                current_p2_ratio = skew_factor
        
            # Handle cases where one subset might get 0 samples if ratio is 1.0 or 0.0
            if current_p2_ratio <= 0.0:
                X_l_p1, y_l_p1 = X_label, y_label
                X_l_p2, y_l_p2 = np.array([], dtype=X.dtype).reshape(0, X.shape[1]), np.array([], dtype=y.dtype)
            elif current_p2_ratio >= 1.0:
                X_l_p1, y_l_p1 = np.array([], dtype=X.dtype).reshape(0, X.shape[1]), np.array([], dtype=y.dtype)
                X_l_p2, y_l_p2 = X_label, y_label
            elif len(y_label) == 1: # Cannot split 1 sample with train_test_split
                 # Assign it based on the ratio probabilistically or deterministically
                 if np.random.rand() < current_p2_ratio: # Assign to P2
                     X_l_p1, y_l_p1 = np.array([], dtype=X.dtype).reshape(0, X.shape[1]), np.array([], dtype=y.dtype)
                     X_l_p2, y_l_p2 = X_label, y_label
                 else: # Assign to P1
                     X_l_p1, y_l_p1 = X_label, y_label
                 ...
                 # Split only the data for the current label
                 X_l_p1, X_l_p2, y_l_p1, y_l_p2 = train_test_split(
                     X_label, y_label,
                     test_size=current_p2_ratio,
                     random_state=seed + i # Add label index for varying state
                     # No stratification needed here as y_label is all one class
                 )
        
            # Append results for this label
            if X_l_p1.shape[0] > 0:
                X_p1_list.append(X_l_p1)
                y_p1_list.append(y_l_p1)
            if X_l_p2.shape[0] > 0:
                X_p2_list.append(X_l_p2)
                y_p2_list.append(y_l_p2)
        
        # Combine lists into final arrays
        X_p1 = np.concatenate(X_p1_list, axis=0) if X_p1_list else np.array([], dtype=X.dtype).reshape(0, X.shape[1])
        y_p1 = np.concatenate(y_p1_list, axis=0) if y_p1_list else np.array([], dtype=y.dtype)
        X_p2 = np.concatenate(X_p2_list, axis=0) if X_p2_list else np.array([], dtype=X.dtype).reshape(0, X.shape[1])
        y_p2 = np.concatenate(y_p2_list, axis=0) if y_p2_list else np.array([], dtype=y.dtype)
        
        # Shuffle the combined datasets
        p1_indices = np.random.permutation(len(X_p1))
        X_p1, y_p1 = X_p1[p1_indices], y_p1[p1_indices]
        p2_indices = np.random.permutation(len(X_p2))
        X_p2, y_p2 = X_p2[p2_indices], y_p2[p2_indices]
